...
void Graph::BFS(int current){
  ...
  // We have to do current - 1 so we match with vecotr indexes
  // only this one is not mathced. Everything else is mathced 
  // index
  NodeHolder[current - 1].Visited = true;
  while(!BFSqueue.empty()){
    current = BFSqueue.front();
    BFSqueue.pop();
    /*
      coyr << current << endl;
      cout << "CURRENT NODE: " << NodeHolder[current - 1].NodeNumber << endl;
      cout << "Current Visited: " << NodeHolder[current - 1].Visited << endl;
    */
    // Go through the vector that holds all the links to other nodes 
    // and cehck the node if visited dont add to queue. vice versa
    for(int i = 0; i < (signed)NodeHolder[current - 1].PathTo.size();i++){
      /*
      cout << "Node in path checking ... " << endl;
      cout << "     Node " << NodeHolder[current - 1].PathTo[i] << endl;
      cout << "     Visited: " << NodeHolder[NodeHolder[current - 1].PathTo[i]].Visited << endl;
      */
      // checking if visited the Node or not
      if(!NodeHolder[NodeHolder[current - 1].PathTo[i]].Visited){
	// also check if intial city or not
	// for some reason it kept being added. 
	// This if statement stopped it 
	if(startCity != NodeHolder[current - 1].PathTo[i]){
	int num = NodeHolder[current - 1].NodeNumber;
	// take the current node and add it to node(index)
	// that is being checked. Also the cost 
	Parent[NodeHolder[current - 1].PathTo[i]] = num;
	Cost[NodeHolder[current - 1].PathTo[i]] = NodeHolder[current - 1].PathCost[i];
	// set the node to visited and add to queue
	NodeHolder[NodeHolder[current - 1].PathTo[i]].Visited = true;
	BFSqueue.push(NodeHolder[current - 1].PathTo[i]);}
      }
    }
  }
  /*
...
int Graph::FindPath(int tourists){
  ...
  int lowest = FinalCost[0];
  // finding the lowest of them all (bottle neck for path)
  for(int i = 1;i<(signed)FinalCost.size();i++){
    if(lowest > FinalCost[i] && FinalCost[i] != -1){
      lowest = FinalCost[i];
    }
  }

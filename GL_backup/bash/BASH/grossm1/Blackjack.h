 ...
 * Author:  John Park (with a few edits from Student Gregory Rossman)
 * Date:    3/10/14
 * Section: all                (Greg Rossman's section: 12)
 * E-mail:  park@umbc.edu     (student email: grossm1@umbc.edu)
 *
...
class Blackjack {
  ...
  private:
    ...
    Deck m_deck; // the deck of cards to be dealt from
    std::vector<Player> m_players; // all the game players
    Player m_dealer; // the dealer, kept separately from players

    static const int m_dealerFunds = 0; // dummy value for construction
                                        // of m_dealer

    static const int m_defaultFunds = 100; // Default money for
                                           // instantiation of player
    int m_numPlayers; // number of players (not counting dealer)

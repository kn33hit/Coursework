...
class Blackjack {
  ...
  private:
    Deck m_deck;                    //Deck object for the current game round
    std::vector<Player> m_players;  //holds players but NOT DEALER
    Player m_dealer;                //declares the dealer Player
    bool m_validBet;                //check for a valid player bet
    bool m_isBust;                  //check for player bust
    int m_playerHandValue;          //player's hand value
    int m_dealerHandValue;          //dealer's hand value
    int m_playerHandSize;           //player's hand size
    int m_dealerHandSize;           //dealer's hand size
    bool m_playerBust;              //player bust check - diff from other check
    bool m_dealerBust;              //dealer bust check - diff from other check
    int m_blackjackHandValue;       //hand value for "blackjack" (21)
    int m_blackjackHandSize;        //hand size for "blackjack" (2)
    bool m_playerLose;              //check for player loss to dealer
    bool m_playerTie;               //check for player tie with dealer
    Card m_tempCard;                //temporary Card object for dealing/hitting
};
